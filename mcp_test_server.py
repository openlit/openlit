...
def create_test_server() -> MockMCPServer:
    ...
    server.add_tool("text_analyzer", "Analyze text and return statistics", text_analyzer)
    ...
    # Register prompts
    server.add_prompt("analysis", "Generate analysis prompt for various topics", analysis_prompt)
...
class MCPTestRunner:
    ...
    async def run_comprehensive_test(self):
        ...
        try:
            ...
            # Text analyzer tool
            text_result = await self.server.call_tool("text_analyzer", {
                "text": "This is a comprehensive test of the text analysis tool functionality."
            })
            print(f"  ✅ Text Analyzer: {text_result}")
            
            # Data processor tool
            data_result = await self.server.call_tool("data_processor", {
                "data": [10, 20, 30, 40, 50],
                "operation": "average"
            })
        ...
        try:
            ...
            print(f"  ✅ Document content: {len(doc_content['contents'][0]['text'])} chars")
            
            config_content = await self.server.read_resource("file://config.json")
            print(f"  ✅ Config content: {len(config_content['contents'][0]['text'])} chars")
        ...
        try:
            ...
            analysis_prompt = await self.server.get_prompt("analysis", {"topic": "business data"})
            print(f"  ✅ Analysis prompt: {analysis_prompt['description']}")
            
            summary_prompt = await self.server.get_prompt("summary", {"content_type": "research paper"})
