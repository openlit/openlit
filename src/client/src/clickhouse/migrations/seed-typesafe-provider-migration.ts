...
const MIGRATION_ID = "seed-typesafe-provider";
 ...
 * static registries later. Insert TypeSafe (and only TypeSafe) when it
 * is missing, without overwriting user-edited rows.
 */
export default async function SeedTypeSafeProviderMigration(
	...
	if (!((existingProviders as { provider_id: string }[]) || []).length) {
		...
		if (insertProviderErr) {
			console.error("Error seeding TypeSafe provider metadata:", insertProviderErr);
			return { err: getMessage().OPERATION_FAILED };
		}
	}
	...
	const toSeed = models
		...
		.map((model) => ({
			...
			model_type: model.modelType || "evaluation",
	...
	if (toSeed.length > 0) {
		...
		if (seedErr) {
			console.error("Error seeding TypeSafe models:", seedErr);
			return { err: getMessage().OPERATION_FAILED };
		}
	}
	...
	return { data: "TypeSafe provider seed successful" };
}
