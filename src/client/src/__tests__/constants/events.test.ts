...
describe("CLIENT_EVENTS", () => {
	...
	it("has prompt events", () => {
		...
		expect(CLIENT_EVENTS.PROMPT_IMPROVEMENT_RUN_SUCCESS).toBe(
			"PROMPT_IMPROVEMENT_RUN_SUCCESS"
		);
		expect(CLIENT_EVENTS.PROMPT_IMPROVEMENT_RUN_FAILURE).toBe(
			"PROMPT_IMPROVEMENT_RUN_FAILURE"
		);
		expect(CLIENT_EVENTS.PROMPT_IMPROVEMENT_SUGGESTION_ACCEPTED).toBe(
			"PROMPT_IMPROVEMENT_SUGGESTION_ACCEPTED"
		);
		expect(CLIENT_EVENTS.PROMPT_IMPROVEMENT_SUGGESTION_DECLINED).toBe(
			"PROMPT_IMPROVEMENT_SUGGESTION_DECLINED"
		);
	});
	...
	it("has inventory-on-view count events", () => {
		expect(CLIENT_EVENTS.AGENTS_HUB_VIEWED).toBe("AGENTS_HUB_VIEWED");
		expect(CLIENT_EVENTS.CODING_AGENTS_VIEWED).toBe("CODING_AGENTS_VIEWED");
		expect(CLIENT_EVENTS.PROMPT_HUB_LIST).toBe("PROMPT_HUB_LIST");
		expect(CLIENT_EVENTS.RULE_ENGINE_LIST).toBe("RULE_ENGINE_LIST");
		expect(CLIENT_EVENTS.EVALUATION_TYPES_LIST).toBe("EVALUATION_TYPES_LIST");
		expect(CLIENT_EVENTS.OBSERVABILITY_PAGE_VISITED).toBe(
			"OBSERVABILITY_PAGE_VISITED"
		);
	});
	...
	it("has otter and ai analysis events", () => {
		expect(CLIENT_EVENTS.OTTER_CHAT_PAGE_VISITED).toBe(
			"OTTER_CHAT_PAGE_VISITED"
		);
		expect(CLIENT_EVENTS.MEMORY_PAGE_VISITED).toBe("MEMORY_PAGE_VISITED");
		expect(CLIENT_EVENTS.OTTER_CHAT_MESSAGE_SENT).toBe(
			"OTTER_CHAT_MESSAGE_SENT"
		);
		expect(CLIENT_EVENTS.OTTER_CHAT_MESSAGE_SUCCESS).toBe(
			"OTTER_CHAT_MESSAGE_SUCCESS"
		);
		expect(CLIENT_EVENTS.OTTER_CHAT_MESSAGE_FAILURE).toBe(
			"OTTER_CHAT_MESSAGE_FAILURE"
		);
		expect(CLIENT_EVENTS.OTTER_CHAT_QUERY_EXECUTED).toBe(
			"OTTER_CHAT_QUERY_EXECUTED"
		);
		expect(CLIENT_EVENTS.OTTER_CHAT_QUERY_EXECUTION_FAILURE).toBe(
			"OTTER_CHAT_QUERY_EXECUTION_FAILURE"
		);
		expect(CLIENT_EVENTS.OTTER_USAGE_PAGE_VISITED).toBe(
			"OTTER_USAGE_PAGE_VISITED"
		);
		expect(CLIENT_EVENTS.OTTER_SETTINGS_PAGE_VISITED).toBe(
			"OTTER_SETTINGS_PAGE_VISITED"
		);
		expect(CLIENT_EVENTS.AI_ANALYSIS_VIEWED).toBe("AI_ANALYSIS_VIEWED");
		expect(CLIENT_EVENTS.AI_ANALYSIS_RUN_SUCCESS).toBe(
			"AI_ANALYSIS_RUN_SUCCESS"
		);
		expect(CLIENT_EVENTS.AI_ANALYSIS_RUN_FAILURE).toBe(
			"AI_ANALYSIS_RUN_FAILURE"
		);
	});
	...
	it("has organisation events", () => {
		...
		expect(CLIENT_EVENTS.ORGANISATION_LIST).toBe("ORGANISATION_LIST");
	});

	it("has project events", () => {
		expect(CLIENT_EVENTS.PROJECT_CREATED).toBe("PROJECT_CREATED");
		expect(CLIENT_EVENTS.PROJECT_SWITCHED).toBe("PROJECT_SWITCHED");
		expect(CLIENT_EVENTS.PROJECT_LIST).toBe("PROJECT_LIST");
	});
...
describe("SERVER_EVENTS", () => {
	...
	it("has otter server events", () => {
		expect(SERVER_EVENTS.OTTER_CHAT_MESSAGE_SUCCESS).toBe(
			"OTTER_CHAT_MESSAGE_SUCCESS"
		);
		expect(SERVER_EVENTS.OTTER_CHAT_MESSAGE_FAILURE).toBe(
			"OTTER_CHAT_MESSAGE_FAILURE"
		);
		expect(SERVER_EVENTS.AI_ANALYSIS_RUN_SUCCESS).toBe(
			"AI_ANALYSIS_RUN_SUCCESS"
		);
		expect(SERVER_EVENTS.AI_ANALYSIS_RUN_FAILURE).toBe(
			"AI_ANALYSIS_RUN_FAILURE"
		);
		expect(SERVER_EVENTS.PROMPT_IMPROVEMENT_SUCCESS).toBe(
			"PROMPT_IMPROVEMENT_SUCCESS"
		);
		expect(SERVER_EVENTS.PROMPT_IMPROVEMENT_FAILURE).toBe(
			"PROMPT_IMPROVEMENT_FAILURE"
		);
	});
	...
	it("has auto pricing events", () => {
		expect(SERVER_EVENTS.PRICING_CONFIG_UPDATED).toBe("PRICING_CONFIG_UPDATED");
		expect(SERVER_EVENTS.PRICING_AUTO_RUN_SUCCESS).toBe(
			"PRICING_AUTO_RUN_SUCCESS"
		);
		expect(SERVER_EVENTS.PRICING_AUTO_RUN_FAILURE).toBe(
			"PRICING_AUTO_RUN_FAILURE"
		);
	});
