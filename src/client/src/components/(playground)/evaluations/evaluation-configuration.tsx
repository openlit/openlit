...
export default function EvaluationConfiguration() {
	const m = getMessage();
	...
	const handleSave = () => {
		if (!provider || !model || !vaultId) {
			toast.error(m.EVALUATION_CONFIG_INVALID, {
		...
		if (autoEvaluation) {
			if (
				...
				toast.error(m.EVALUATION_SAMPLE_RATE_PERCENT_INVALID, {
					id: EVALUATION_TOAST_ID,
				});
				return;
			}
		...
		toast.loading(m.EVALUATION_CONFIG_MODIFYING, {
			id: EVALUATION_TOAST_ID,
		});
		...
		saveConfig({
			...
			successCb: () => {
				toast.success(
					config?.id ? m.EVALUATION_UPDATED : m.EVALUATION_CREATED,
					{ id: EVALUATION_TOAST_ID }
				);
				getConfig({
					requestType: "GET",
			...
			failureCb: (err?: string) => {
				toast.error(err || m.EVALUATION_CONFIG_UPDATING_FAILED, {
					id: EVALUATION_TOAST_ID,
				});
			},
		});
					...
					<div className="flex gap-3">
						...
						<div className="text-sm text-stone-600 dark:text-stone-400">
							<p className="font-medium text-stone-700 dark:text-stone-300 mb-1">
								{m.EVALUATION_MANUAL_AND_AUTO}
							</p>
							<p>{m.EVALUATION_MANUAL_AND_AUTO_DESCRIPTION}</p>
						</div>
					</div>
				</CardContent>
			</Card>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
					...
					<Card className="border-stone-200 dark:border-stone-800 shadow-sm">
						...
						<CardContent className="space-y-4">
							...
							<div className="grid gap-3">
								...
								>
									<SelectTrigger>
										<SelectValue placeholder={m.EVALUATION_SELECT_PROVIDER} />
									</SelectTrigger>
									<SelectContent>
										{(providers || []).map((p) => (
							...
							<div className="grid gap-3">
								<Label>{m.EVALUATION_MODEL_LABEL}</Label>
								<Select
									value={model}
								...
								>
									<SelectTrigger className="h-auto">
										<SelectValue
											placeholder={
												provider
													? m.EVALUATION_MODEL_PLACEHOLDER
													: m.EVALUATION_SELECT_PROVIDER_FIRST
											}
										/>
									</SelectTrigger>
									<SelectContent>
											...
											<SelectItem key={modelOption.id} value={modelOption.id}>
												<div className="flex flex-col items-start gap-0.5">
													...
													<div className="flex gap-1.5 text-xs text-stone-500">
														...
														>
															{modelOption.contextWindow.toLocaleString()}
														</Badge>
														<Badge
															variant="outline"
															className="text-xs h-4 px-1"
														>
															${modelOption.inputPricePerMToken}/M
														</Badge>
													</div>
												</div>
											</SelectItem>
							...
							<div className="grid gap-3">
								<Label>{m.EVALUATION_API_KEY_VAULT}</Label>
								<Select value={vaultId} onValueChange={setVaultId}>
									<SelectTrigger>
										<SelectValue placeholder={m.EVALUATION_SELECT_VAULT_KEY} />
									</SelectTrigger>
									<SelectContent>
										{vaultKeys.map((o) => (
				...
				<div className="grid gap-4 content-start">
					<Card className="border-stone-200 dark:border-stone-800 shadow-sm">
						<CardHeader className="pb-4">
							<CardTitle className="text-base flex items-center gap-2">
								...
								{m.EVALUATION_AUTO_TITLE}
							</CardTitle>
							<p className="text-sm text-stone-500 dark:text-stone-400 font-normal">
								{m.EVALUATION_AUTO_DESCRIPTION}
							</p>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="flex items-center justify-between rounded-lg border border-stone-200 dark:border-stone-700 p-4">
								<div className="space-y-0.5">
									<Label>{m.EVALUATION_ENABLE_AUTO}</Label>
									<p className="text-xs text-stone-500 dark:text-stone-400">
										{m.EVALUATION_ENABLE_AUTO_DESCRIPTION}
									</p>
								</div>
								<Switch
							...
							{autoEvaluation && (
								<div className="grid gap-3">
									<Label className="flex items-center gap-2">
										...
										{m.EVALUATION_CRON_SCHEDULE}
									</Label>
									<Input
										placeholder={m.EVALUATION_CRON_PLACEHOLDER}
										value={recurringTime}
										onChange={(e) => setRecurringTime(e.target.value)}
									/>
									<p className="text-xs text-stone-500 dark:text-stone-400">
										{m.EVALUATION_CRON_HELP}
									</p>
									<Label>{m.EVALUATION_SAMPLE_RATE_LABEL}</Label>
									<Input
										type="number"
										min={0}
										max={100}
									...
									/>
									<p className="text-xs text-stone-500 dark:text-stone-400">
										{m.EVALUATION_SAMPLE_RATE_DESCRIPTION}
									</p>
								</div>
							)}
						</CardContent>
					...
					<Button
						onClick={handleSave}
						disabled={isSaving}
						className="w-full sm:w-auto bg-primary dark:bg-primary text-white dark:text-white hover:bg-primary/90 dark:hover:bg-primary/90"
					>
						{isSaving
							? m.EVALUATION_SAVING
							: config?.id
								? m.EVALUATION_SAVE_CHANGES
								: m.EVALUATION_CREATE_CONFIG}
					</Button>
					...
					<Card className="border-stone-200 dark:border-stone-800 shadow-sm">
						<CardHeader className="pb-4">
							<CardTitle className="text-base flex items-center gap-2">
								...
								{m.EVALUATION_MANUAL_TITLE}
							</CardTitle>
							<p className="text-sm text-stone-500 dark:text-stone-400 font-normal">
								{m.EVALUATION_MANUAL_DESCRIPTION}
							</p>
						</CardHeader>
						<CardContent className="space-y-4">
							<ol className="list-decimal list-inside space-y-2 text-sm text-stone-600 dark:text-stone-400 mb-3">
								<li>{m.EVALUATION_MANUAL_STEP_1}</li>
								<li>{m.EVALUATION_MANUAL_STEP_2}</li>
								<li>{m.EVALUATION_MANUAL_STEP_3}</li>
							</ol>
							<Link href="/telemetry">
								<Button variant="default">{m.EVALUATION_GO_TO_TRACES}</Button>
							</Link>
						</CardContent>
					</Card>
				</div>
