...
const envsHandler: FeatureHandler = {
	feature: FEATURE,
	...
	async applyOperation(
		...
		try {
			...
			return Response.json(
				{ error: error.message || "ENVs operation failed" },
				{ status: 500 }
			);
		}
	},
	...
	reconcile(
		...
		// controller-side ENV delivery mechanism is built.
		return [];
	},
};

registerFeature(envsHandler);

export default envsHandler;
