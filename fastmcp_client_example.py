...
async def test_fastmcp_server():
    ...
    server_params = StdioServerParameters(
        command=sys.executable, args=["fastmcp_server_example.py"]
    )
            ...
            async with ClientSession(read_stream, write_stream) as session:
                ...
                if not calc_result.isError:
                    ...
                    if calc_result.content:
                        logger.info(
                            f"📊 Result: {calc_result.content[0].text if calc_result.content else 'N/A'}"
                        )
                else:
                ...
                if resources_result.resources:
                    ...
                    logger.info(
                        f"📊 Content type: {read_result.contents[0].mimeType if read_result.contents else 'N/A'}"
                    )
                ...
                if prompts_result.prompts:
                    ...
                    prompt_result = await session.get_prompt(
                        prompt_name, {"data_type": "financial"}
                    )
                ...
                logger.info(
                    "   ✅ Simplified operation names (tools_call, resources_read, etc.)"
                )
                logger.info(
                    "   ✅ Enhanced span names (mcp fastmcp/run, mcp managers/tool, etc.)"
                )
