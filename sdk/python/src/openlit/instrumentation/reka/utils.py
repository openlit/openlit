...
def build_input_messages(messages):
    """
    Convert Reka request messages to OTel input message structure.
...
def build_output_messages(response_text, finish_reason, tool_calls=None):
    """
    Convert Reka response to OTel output message structure.
...
def common_chat_logic(
    ...
    version_extras = apply_agent_version_attributes(
        ...
        runtime_config={
            ...
            "provider": SemanticConvention.GEN_AI_SYSTEM_REKAAI,
        },
        providers=[SemanticConvention.GEN_AI_SYSTEM_REKAAI],
    )
...
def process_chat_response(
    ...
    **kwargs,
    ...
    responses_list = response_dict.get("responses") or []
    first_response = responses_list[0] if responses_list else {}
    first_message = (
        first_response.get("message", {}) if isinstance(first_response, dict) else {}
    )
    ...
    scope._llmresponse = " ".join(
        (r.get("message", {}).get("content") or "") for r in responses_list
    )
    ...
    scope._output_tokens = usage.get("output_tokens") or usage.get(
        "generated_tokens", 0
    )
    ...
    scope._finish_reason = str(first_response.get("finish_reason", ""))
    if scope._kwargs.get("tools"):
        scope._tools = first_message.get("tool_calls")
    else:
        scope._tools = None
