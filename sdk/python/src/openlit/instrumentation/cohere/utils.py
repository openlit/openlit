...
def common_chat_logic(
    ...
    version_extras = apply_agent_version_attributes(
        ...
        runtime_config={
            ...
            "provider": SemanticConvention.GEN_AI_SYSTEM_COHERE,
        },
        providers=[SemanticConvention.GEN_AI_SYSTEM_COHERE],
    )
    ...
    # Emit inference event independently of content capture so metadata
    # (token counts, finish reason, version hash, etc.) always reach the
    # event stream.
    if event_provider:
        try:
            extra = {
