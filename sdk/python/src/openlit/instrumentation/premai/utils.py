...
def build_input_messages(messages):
    """
    Convert Premai request messages to OTel input message structure.
...
def build_output_messages(response_text, finish_reason, tool_calls=None):
    """
    Convert Premai response to OTel output message structure.
...
def process_chunk(scope, chunk):
    ...
    choices = chunked.get("choices", []) if isinstance(chunked, dict) else []
    if choices:
        first_choice = choices[0] if isinstance(choices[0], dict) else {}
        delta = first_choice.get("delta") or {}
        if isinstance(delta, dict) and delta.get("content"):
            scope._llmresponse += delta.get("content", "")
        if first_choice.get("finish_reason"):
            scope._finish_reason = first_choice.get("finish_reason")
            scope._response_id = chunked.get("id")
            scope._response_model = chunked.get("model")
            scope._end_time = time.time()

    usage = chunked.get("usage") if isinstance(chunked, dict) else None
...
def common_chat_logic(
    ...
    # Premai chat (no additional API type attribute)
    ...
    version_extras = apply_agent_version_attributes(
        ...
        runtime_config={
            ...
            "provider": SemanticConvention.GEN_AI_SYSTEM_PREMAI,
        },
        providers=[SemanticConvention.GEN_AI_SYSTEM_PREMAI],
    )
    ...
    if event_provider:
        try:
            extra = {
                ...
                "temperature": scope._kwargs.get("temperature", 1.0),
...
def process_chat_response(
    ...
    **kwargs,
    ...
    scope._end_time = time.time()
    scope._span = span
    scope._llmresponse = str(message.get("content", ""))
    ...
    if scope._kwargs.get("tools"):
        scope._tools = message.get("tool_calls")
    else:
        scope._tools = None
