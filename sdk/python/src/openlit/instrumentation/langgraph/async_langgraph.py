...
def async_general_wrap(
    ...
    if gen_ai_endpoint == "graph_astream":
        def astream_wrapper(wrapped, instance, args, kwargs):
            ...
            # Get operation type
            operation_type = OPERATION_MAP.get(gen_ai_endpoint, SemanticConvention.GEN_AI_OPERATION_TYPE_FRAMEWORK)

            # Generate span name
            span_name = generate_span_name(operation_type, gen_ai_endpoint, instance, args, kwargs)

            # Return an async generator wrapper
            return _create_async_stream_wrapper(
                wrapped, instance, args, kwargs, span_name, tracer,
                operation_type, server_address, server_port, environment,
                application_name, metrics, capture_message_content,
                disable_metrics, version, gen_ai_endpoint
            )
...
async def _create_async_stream_wrapper(
    ...
    operation_type, server_address, server_port, environment,
    application_name, metrics, capture_message_content,
    disable_metrics, version, endpoint
):
    """
    ...
    with tracer.start_as_current_span(span_name, kind=SpanKind.CLIENT) as span:
        ...
        span.set_attribute(SemanticConvention.GEN_AI_SYSTEM, SemanticConvention.GEN_AI_SYSTEM_LANGGRAPH)
        ...
        if config:
            ...
            if config_info.get("thread_id"):
                span.set_attribute(SemanticConvention.LANGGRAPH_THREAD_ID, config_info["thread_id"])
            if config_info.get("checkpoint_id"):
                span.set_attribute(SemanticConvention.LANGGRAPH_CHECKPOINT_ID, config_info["checkpoint_id"])
        ...
        try:
            ...
            _finalize_async_stream_span(span, execution_state, capture_message_content, start_time)
...
def _finalize_async_stream_span(span, execution_state, capture_message_content, start_time):
    ...
    span.set_attribute(
        SemanticConvention.LANGGRAPH_MESSAGE_COUNT,
        execution_state["message_count"]
    )
    span.set_attribute(
        SemanticConvention.LANGGRAPH_CHUNK_COUNT,
        execution_state["chunk_count"]
    )
            ...
            try:
                ...
                if gen_ai_endpoint == "checkpoint_write":
                    ...
                    if result and hasattr(result, "checkpoint"):
                        ...
                        if checkpoint and hasattr(checkpoint, "id"):
                            span.set_attribute(SemanticConvention.LANGGRAPH_CHECKPOINT_ID, str(checkpoint.id))
                ...
                if not disable_metrics and metrics:
                    _record_checkpoint_metrics(
                        metrics, operation_type, end_time - start_time, environment, application_name
                    )
...
def _record_checkpoint_metrics(metrics, operation_type, duration, environment, application_name):
