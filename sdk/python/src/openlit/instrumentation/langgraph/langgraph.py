            ...
            try:
                ...
                if gen_ai_endpoint == "graph_stream":
                    return _handle_stream(
                        wrapped, instance, args, kwargs, span, start_time,
    ...
    try:
        ...
        def stream_wrapper():
            try:
                ...
                _finalize_stream_span(span, execution_state, capture_message_content, start_time)
            ...
            if inspect.iscoroutinefunction(original_func):
                ...
                async def wrapped_node_async(state, *node_args, **node_kwargs):
                    ...
                    with tracer.start_as_current_span(span_name, kind=SpanKind.CLIENT) as span:
                        start_time = time.time()
                        span.set_attribute(SemanticConvention.GEN_AI_SYSTEM, SemanticConvention.GEN_AI_SYSTEM_LANGGRAPH)
                        span.set_attribute(SemanticConvention.GEN_AI_OPERATION, SemanticConvention.GEN_AI_OPERATION_TYPE_AGENT)
                        span.set_attribute(SemanticConvention.LANGGRAPH_NODE_NAME, node_name)
                        span.set_attribute(SemanticConvention.GEN_AI_ENVIRONMENT, environment)
                        span.set_attribute(SemanticConvention.GEN_AI_APPLICATION_NAME, application_name)
                        
                        try:
                            result = await original_func(state, *node_args, **node_kwargs)
