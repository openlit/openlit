        ...
        def build_config_from_environment():
            ...
            for env_var, param_name in env_mappings.items():
                ...
                if env_value:
                    ...
                    if env_var in ['OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT', 
                                   'OPENLIT_DISABLE_BATCH', 'OPENLIT_DISABLE_METRICS',
                                   'OPENLIT_COLLECT_GPU_STATS', 'OPENLIT_DETAILED_TRACING']:
                        config[param_name] = env_value.lower() in ('true', '1', 'yes')
                    # Handle CSV values
                    elif env_var == 'OPENLIT_DISABLED_INSTRUMENTORS':
                        config[param_name] = [item.strip() for item in env_value.split(',') if item.strip()]
                    # Handle JSON values
...
def auto_initialize_openlit():
    ...
    try:
        ...
        print("❌ OpenLIT auto-instrumentation failed: OpenLIT not installed", file=sys.stderr)
