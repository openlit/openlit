...
"""Regression tests: the VertexAI streaming wrappers must end their span on
every exit path.
...
the whole call (span, cost, tokens) was lost. This is the VertexAI instance of
the early-close streaming-span leak fixed for Anthropic in #1461 and for Groq
in #1516.

These tests drive the real `send_message`/`async_send_message` wrapper
factories with a synthetic VertexAI-shaped stream and assert the span ends
exactly once, carrying token-usage attributes, on each exit path.
"""
...
from openlit.instrumentation.vertexai import vertexai as sync_mod
from openlit.instrumentation.vertexai import async_vertexai as async_mod
from openlit.semcov import SemanticConvention

# stream=True selects the wrapper's streaming path; contents left empty so the
# telemetry is driven purely by the synthetic chunks below.
REQUEST_KWARGS = {"stream": True}


class FakeUsageMetadata:
    """Shaped like vertexai's GenerationResponse.usage_metadata."""
...
class FakeChunk:
    """Shaped like a vertexai streaming chunk (has .text and .usage_metadata)."""
...
def _chunks():
    return iter(
        [
            FakeChunk("partial"),
            FakeChunk(" answer", FakeUsageMetadata()),
        ]
    )


class FakeInstance:
    """Minimal instance so extract_vertexai_details() resolves a model name."""

    def __init__(self):
        self._location = "us-central1"
        self._model_name = (
            "projects/p/locations/us-central1/publishers/google/models/gemini-1.5-flash"
        )
...
def _factory(tracer, *, is_async):
    ...
    make = mod.async_send_message if is_async else mod.send_message
    return make(
        version="test",
        environment="test",
...
class FakeSyncStream:
    """Sync context-manager stream shaped like vertexai's streaming response."""
...
class FakeAsyncStream:
    """Async context-manager stream shaped like vertexai's async streaming response."""
...
async def _acreate(*_a, **_k):
    """Async stand-in for the awaited vertexai async call."""
...
def test_sync_early_break_inside_with_ends_span():
    ...
    stream = wrapper(lambda *a, **k: FakeSyncStream(), FakeInstance(), (), REQUEST_KWARGS)
    with stream as s:
...
async def test_async_early_break_inside_with_ends_span():
    ...
    stream = await wrapper(_acreate, FakeInstance(), (), REQUEST_KWARGS)
