...
describe('OpenLitHelper.getChatModelCost — cache pricing', () => {
    describe('no cache tokens', () => {
        it('matches the legacy formula', () => {
            const cost = helpers_1.default.getChatModelCost(MODEL, PRICING, 1000, 500);
            expect(cost).toBeCloseTo((1000 / 1000) * 0.003 + (500 / 1000) * 0.015, 10);
        });
        it('returns 0 for an unknown model', () => {
            expect(helpers_1.default.getChatModelCost('nope', PRICING, 10, 10)).toBe(0);
        });
        it('returns 0 when pricing info is missing', () => {
            expect(helpers_1.default.getChatModelCost(MODEL, {}, 10, 10)).toBe(0);
        });
    });
    describe('Anthropic convention (promptTokens exclusive of cache)', () => {
        it('adds cache cost on top of the prompt base', () => {
            const cost = helpers_1.default.getChatModelCost(MODEL, PRICING, 800, 500, 100, 100);
            const expected = (800 / 1000) * 0.003 +
                (500 / 1000) * 0.015 +
                (100 / 1000) * 0.0003 +
                (100 / 1000) * 0.00375;
        ...
        it('leaves the prompt base untouched when no cache price exists', () => {
            const cost = helpers_1.default.getChatModelCost('model-no-cache-price', PRICING, 800, 500, 100, 100);
    ...
    describe('OpenAI / LangChain convention (promptTokens inclusive of cache)', () => {
        it('does not bill cache tokens twice', () => {
            ...
            const cost = helpers_1.default.getChatModelCost(MODEL, PRICING, 1000, 500, 100, 100, true);
            const expected = (800 / 1000) * 0.003 +
                (500 / 1000) * 0.015 +
                (100 / 1000) * 0.0003 +
                (100 / 1000) * 0.00375;
        ...
        it('matches the exclusive call describing the same request', () => {
            const inclusive = helpers_1.default.getChatModelCost(MODEL, PRICING, 1000, 500, 100, 100, true);
            const exclusive = helpers_1.default.getChatModelCost(MODEL, PRICING, 800, 500, 100, 100);
            expect(inclusive).toBeCloseTo(exclusive, 10);
        });
        it('matches the legacy total when no cache price exists (no regression)', () => {
            const cost = helpers_1.default.getChatModelCost('model-no-cache-price', PRICING, 1000, 500, 100, 100, true);
            expect(cost).toBeCloseTo((1000 / 1000) * 0.003 + (500 / 1000) * 0.015, 10);
        });
        it('handles OpenAI-style cache read only (no cache creation)', () => {
            ...
            const cost = helpers_1.default.getChatModelCost('gpt-cache', pricing, 1000, 200, 400, 0, true);
            const expected = (600 / 1000) * 0.0025 + (200 / 1000) * 0.01 + (400 / 1000) * 0.00125;
            expect(cost).toBeCloseTo(expected, 10);
        });
        it('clamps the prompt base at 0 for malformed token data', () => {
            const cost = helpers_1.default.getChatModelCost(MODEL, PRICING, 100, 0, 200, 200, true);
            const expected = (200 / 1000) * 0.0003 + (200 / 1000) * 0.00375;
            expect(cost).toBeCloseTo(expected, 10);
        });
    });
});
//# sourceMappingURL=cache-pricing.test.js.map