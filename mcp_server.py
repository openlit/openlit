...
async def read_resource(uri: str) -> ReadResourceResult:
    ...
    if uri == "file://demo.txt":
        ...
        return ReadResourceResult(
            contents=[
                TextContent(type="text", text=f"Error: Resource '{uri}' not found")
            ]
        )
...
async def main():
    ...
    logger.info(
        "🔍 Server operations will generate telemetry spans with detailed_tracing=True"
    )
    ...
    try:
        ...
        async with stdio_server() as (read_stream, write_stream):
            ...
            logger.info(
                "🔌 Server running - clients can connect via stdio transport..."
            )
