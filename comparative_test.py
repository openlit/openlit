...
asyncio.run(main())
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                data_result = await session.call_tool("data_processor", {"data": [10, 20, 30, 40, 50]})
                ...
                analysis_prompt = await session.get_prompt("analysis", {"topic": "performance"})
                
                print("  📄 Getting summary prompt...")
                summary_prompt = await session.get_prompt("summary", {"type": "technical"})
...
def analyze_results(results: List[Dict[str, Any]]):
    ...
    for result in results:
        if result:
            ...
            key_attrs = len([k for k in attrs.keys() if k.startswith(('mcp.', 'gen_ai.', 'openinference.'))])
    ...
    print(f"\n🏆 COMPETITIVE ANALYSIS:")
    openlit_result = next((r for r in results if r and r['instrumentation'] == 'OpenLIT'), None)
