...
resource = Resource.create({
    "service.name": "mcp-server-openllmetry",
...
async def call_tool(name: str, arguments: dict) -> CallToolResult:
    ...
    logger.info(f"🔧 OpenLLMetry Server: Executing tool '{name}' with args: {arguments}")
    ...
    if name == "calculator":
        ...
        if operation == "add":
            ...
            if b == 0:
                return CallToolResult(
                    content=[TextContent(type="text", text="Error: Division by zero (OpenLLMetry)")]
                )
            result = a / b
        else:
            return CallToolResult(
                content=[TextContent(type="text", text=f"Error: Unknown operation '{operation}' (OpenLLMetry)")]
            )
        ...
        return CallToolResult(
            content=[TextContent(type="text", text=f"OpenLLMetry Generated UUID: {new_uuid}")]
        )
    
    else:
        return CallToolResult(
            content=[TextContent(type="text", text=f"Error: Unknown tool '{name}' (OpenLLMetry)")]
        )
...
async def read_resource(uri: str) -> ReadResourceResult:
    ...
    if uri == "file://openllmetry-demo.txt":
        ...
        return ReadResourceResult(
            contents=[TextContent(type="text", text=f"Error: Resource '{uri}' not found (OpenLLMetry)")]
        )
