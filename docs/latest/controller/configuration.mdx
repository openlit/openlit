...
The Controller is configured via a YAML file (pass its path with `--config`) or through environment variables. It also accepts two command-line flags:

```bash
openlit-controller --config /etc/openlit/config.yaml --openlit-url http://openlit:3000
```

| Flag | Description |
|---|---|
| `--config` | Path to the YAML config file. Omit to run from environment variables only. |
| `--openlit-url` | OpenLIT dashboard URL. Overrides `OPENLIT_URL` / the `openlit_url` config field. |
...
| `poll_interval` | `OPENLIT_POLL_INTERVAL` | `60s` | Duration between poll cycles (e.g., `60s`, `2m`). The dashboard may override this per poll, clamped to the `5s`–`300s` range. |
| `environment` | `OPENLIT_ENVIRONMENT` | `default` | Deployment environment label (e.g., `production`, `staging`). Falls back to `OTEL_DEPLOYMENT_ENVIRONMENT`. |
...
| `obi_binary_path` | `OPENLIT_OBI_PATH` | `/usr/local/bin/obi` | Path to the bundled OBI (eBPF) binary |
| `proc_root` | `OPENLIT_PROC_ROOT` | `/proc` | Root of the `/proc` filesystem to scan. Set to `/host/proc` when `/proc` is mounted there (e.g., the Kubernetes DaemonSet). |
| `sdk_version` | `OPENLIT_SDK_VERSION` | (auto) | Pin a specific OpenLIT Python SDK version (validated against the PEP 440 version grammar before injection) |
| — | `OPENLIT_DEPLOY_MODE` | (auto) | Force the deployment mode: `kubernetes`, `docker`, or `linux` |
...
The Controller auto-detects its deployment mode on startup:

- **Kubernetes** — Detected when the in-cluster service account is mounted at `/var/run/secrets/kubernetes.io/serviceaccount`
