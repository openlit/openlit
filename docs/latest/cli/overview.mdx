...
The **`openlit` CLI** is a single Go binary that connects your local tooling to an OpenLIT instance. Today it ships first-class observability for AI **coding agents** - Claude Code, Cursor, Codex, and OpenCode - by wiring into each vendor's hook system, normalizing events onto OpenTelemetry's `gen_ai.*` and OpenLIT's `coding_agent.*` semantic conventions, and exporting them via OTLP to your collector.
...
<CardGroup cols={2}>
  <Card title="Observe coding agents" icon="code">
    Wire Claude Code, Cursor, Codex, or OpenCode into OpenLIT with one idempotent command per vendor - no SDK, no code changes in your repos.
  </Card>
...
| Command | Purpose |
|---|---|
| [`openlit coding`](/latest/cli/commands#openlit-coding) | Install, launch, and manage coding-agent telemetry (Claude Code, Cursor, Codex, OpenCode) |
