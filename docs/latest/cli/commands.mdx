...
```
openlit
├── coding        Coding-agent observability (Claude Code, Cursor, Codex, OpenCode)
...
Coding-agent observability for Claude Code, Cursor, Codex, and OpenCode. See the [Coding Agents setup guide](/latest/openlit/coding-agents/setup-and-configure) for the full vendor-by-vendor walkthrough.
...
| Flag | Description |
|---|---|
| `--vendor` *(required)* | `claude-code` \| `cursor` \| `codex` \| `opencode` \| `all` |
...
| OpenCode | `$XDG_CONFIG_HOME/opencode/plugins/openlit.ts` when set; otherwise `<user-home>/.config/opencode/plugins/openlit.ts` |
...
|---|---|
| `<agent>` *(required)* | `claude` \| `claude-code` \| `cc` \| `cursor` \| `codex` \| `opencode` (`claude-code` and `cc` are aliases for `claude`) |
...
| Flag | Description |
|---|---|
| `--vendor` *(required)* | `cc` \| `claude-code` \| `cursor` \| `codex` \| `opencode` |
