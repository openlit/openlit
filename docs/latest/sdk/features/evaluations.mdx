...
Use LLM-based detection with providers like OpenAI, Anthropic, or Forge:
...
|---------------------|---------------------------------------------------------------------------------------------------|---------------|-------------------------|
| `provider`          | The name of the LLM provider, either `"openai"` or `"anthropic"`. To use Forge, set `FORGE_API_KEY` (and optional `FORGE_API_BASE`) with `provider="openai"`. | `"openai"`    | `"openai"`              |
