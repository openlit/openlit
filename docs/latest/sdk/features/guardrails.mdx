...
<Tabs>
  <Tab title="LLM-Based Detection (Python)">
    ...
    With LLM-based detection, you can use providers like OpenAI, Anthropic, or Forge. Alternatively, you can specify a `base_url` with `provider="openai"` to use any provider that is compatible with the OpenAI SDK.
  ...
  <Tab title="LLM-Based Detection (TypeScript)">
    ...
    With LLM-based detection, you can use providers like OpenAI, Anthropic, or Forge. You can also specify a `baseUrl` with `provider: "openai"` to use any provider compatible with the OpenAI SDK.
...
<AccordionGroup>
    ...
    OpenAI-compatible (e.g., OpenAI/gpt-4o-mini)
    </Accordion>
...
|---------------------|---------------------------------------------------------------------------------------------------|---------------|-------------------------|
| `provider`          | The LLM provider name, either `"openai"` or `"anthropic"`. To use Forge, set `FORGE_API_KEY` (and optional `FORGE_API_BASE`) with `provider="openai"`. Omitting this with `custom_rules` uses regex detection without an LLM. | `None`        | `"openai"`              |
| `api_key`           | API key for LLM authentication, set via `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `FORGE_API_KEY` environment variables. | `None`        | `os.getenv("OPENAI_API_KEY")` |
...
2. **Detection Choice**:
   ...
   - **LLM Detection**: Uses an LLM (OpenAI, Anthropic, or Forge) if a provider is specified.
...
<Tabs>
  <Tab title="LLM-Based Detection (Python)">
    ...
    With LLM-based detection, you can use providers like OpenAI, Anthropic, or Forge. Alternatively, you can specify a `base_url` with `provider="openai"` to use any provider compatible with the OpenAI SDK.
...
2. **Detection Choice**:
   ...
   - **LLM Detection**: Utilizes an LLM (OpenAI, Anthropic, or Forge) if a provider is specified.
...
| Name                | Description                                                                                       | Default Value | Example Value           |
|---------------------|---------------------------------------------------------------------------------------------------|---------------|-------------------------|
| `provider`          | The LLM provider name, either `"openai"` or `"anthropic"`. To use Forge, set `FORGE_API_KEY` (and optional `FORGE_API_BASE`) with `provider="openai"`. | `None`        | `"openai"`              |
| `api_key`           | API key for LLM authentication, set via `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `FORGE_API_KEY` environment variables. | `None`        | `os.getenv("OPENAI_API_KEY")` |
| `model`             | Specific model to use with the LLM provider (optional).                                           | `None`        | `"gpt-4o"`              |
| `base_url`          | Base URL for the LLM API (optional).                                                              | `None`        | `"https://api.openai.com/v1"` |
...
3. **LLM Evaluation**: Utilizes the LLM (OpenAI, Anthropic, or Forge) to evaluate the text against the provided topic constraints.
...
<Tabs>
  <Tab title="LLM-Based Detection">
    ...
    With LLM-based detection, you can use providers like OpenAI, Anthropic, or Forge. Alternatively, specify a `base_url` with `provider="openai"` to use any provider compatible with the OpenAI SDK.
