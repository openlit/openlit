...
<CardGroup cols={2}>
  ...
  <Card title="PII / Data Redaction" href="#pii-data-redaction">
...
<AccordionGroup>
<Accordion title="`openlit.PII()` Class Parameters">

| Name | Description | Default Value | Example Value |
|------|-------------|----------------|----------------|
| `action` | What to do when PII is detected: `"redact"` replaces matches, `"deny"` blocks the call, `"warn"` logs without blocking. | `"redact"` | `"deny"` |
| `custom_patterns` | Additional regex patterns to detect, as a `{label: regex}` mapping. Combined with the built-in patterns. | `None` | `{"internal_id": r"INT-\d{6}"}` |
