...
## Using an existing OTel TracerProvider

If your application already configures OpenTelemetry, OpenLIT reuses it instead of creating its own.
Set your SDK `TracerProvider` as the global provider **before** calling `openlit.init()`; OpenLIT detects it and attaches its instrumentation to that provider.

`openlit.init()` does not take a `tracer` argument.

When an existing provider is reused, OpenLIT does not add exporters to it, so `otlp_endpoint` / `otlp_headers` have no effect. Configure exporters on your own `TracerProvider`.
...
provider = TracerProvider(...)  # your existing configuration
