---
title: 'Monitor DigitalOcean Gradient using OpenTelemetry'
description: "OpenLIT auto-instruments DigitalOcean's Gradient AI Platform via the gradient SDK, giving AI observability with cost tracking across chat, image, and RAG calls."
sidebarTitle: 'DigitalOcean Gradient'
---

import IntegrationMethods from '/snippets/integration-methods.mdx';

OpenLIT uses OpenTelemetry Auto-Instrumentation to help you monitor LLM applications built on DigitalOcean's Gradient AI Platform via the [`gradient`](https://github.com/digitalocean/gradient-python) SDK. This includes tracking performance, token usage, costs, and how users interact with chat completions, the OpenAI-style Responses API, image generation, DigitalOcean Agents, and knowledge-base retrieval (RAG).
...
- DigitalOcean `gradient` Python SDK `>= 3.0.0` (sync `Gradient` and async `AsyncGradient` clients)
