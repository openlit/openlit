...
Configure OpenLIT using either the SDK (`openlit.init()`) or CLI (`openlit-instrument`) approach:

### SDK Integration (`openlit.init()`)

For applications where you can modify code, use the SDK approach:
...
### Zero Code Changes (`openlit-instrument`)

For existing applications without code modifications:
...
# With configuration
openlit-instrument --service-name my-ai-app --environment production python your_app.py
