...
Vault features allows you to create secrets, update secrets and set secrets in your application environments using our sdk

## Key Features

- **Vault Management**: Create, edit, and track different secrets of your applications.
- **Vault Access**: Access all secrets or on the basis of key, tags and set the secrets in your node/python environments.
    ...
    <Steps>
      ...
      <Step title="Get secret(s) using the SDK">
        <Tabs>
          ...
          <Tab title="API">
            To retrieve the secret via an API request, Refer to the [**GET Secret(s)**](/latest/api-reference/endpoint/vault/get) API Reference Documentation.
          </Tab>
        </Tabs>
      </Step>
    </Steps>
