...
This document outlines the OpenTelemetry traces collected by **OpenLIT** from applications utilizing Large Language Models (LLMs) and Vector Databases. The span attributes adhere to the [GenAI Semantic Conventions](https://github.com/open-telemetry/semantic-conventions/tree/main/docs/gen-ai) established by the OpenTelemetry community, ensuring standardized data collection that enhances monitoring and debugging capabilities.
...
| `gen_ai.type`                   | string   | The type of LLM operation performed.             | `chat`, `embedding`, `audio`, `image`, `fine_tuning`, `vectordb`, `framework` |
...
|-------------------------------------|---------|----------------------------------------------------------------|-----------------------------------------------|
| `gen_ai.content.prompt`             | string  | The full prompt sent to the LLM.                               | `"What is the capital of France?"`            |
| `gen_ai.content.completion`         | string  | The full response received from the LLM.                       | `"The capital of France is Paris."`           |
