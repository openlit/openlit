...
Using `openlit.trace`, you get access to advanced function tracing capabilities, allowing you to record every process within a single function.
...
def confirm_one_liner():
    with openlit.start_trace("Confirm One Liner") as trace:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"Correct guess? One-liner:{one_liner},Guess:{guess}",
                }
            ],
        )
        trace.set_result(completion.choices[0].message.content)
...
if __name__ == "__main__":
    openlit.init(environment="development", application_name="test-app")

    print(confirm_one_liner())

```

Here's how it displays in [Murnitur.ai](https://murnitur.ai)

<img style={{ borderRadius: "0.5rem" }} src="/images/full-trace.png" />
