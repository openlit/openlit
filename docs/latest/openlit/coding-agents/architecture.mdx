...
[Onboarding](/latest/openlit/coding-agents/onboarding) page is what you
...
| `chat_id` (UI) | Derived: foreign `parent_id` → else `conversation.id` → else `session.id` (self-parent echoes ignored) | One chat thread including its linked subagents |
...
Cause: a subagent / Task run reports its own `session_id` but not the
parent's `parent_conversation_id`, so the chat-thread coalesce falls
through to per-process session_id. Cursor often omits
`parent_conversation_id` on short Task sessions, so they show as
orphan rows next to the main chat. Linked subagents (foreign
`coding_agent.agent.parent_id`) fold under the parent and surface as a
subagent badge on that row.

Fix: confirm `peekContext` is picking the vendor's parent-id field,
and that `foreignParentID` in `hook.go` promotes only a *foreign*
parent (never `parent_id == conversation.id`, which Cursor echoes on
parent-chat hooks). OpenLIT will not invent parent links when the
vendor does not send them.

### Overview shows N sessions but Sessions lists N−1

Cause: Cursor stamped `parent_conversation_id` equal to the chat's own
id on parent-chat hooks ("self-parent"). The Sessions list used to
treat any non-empty `parent_id` as `is_subagent=1`, which hid the
main chat while Overview still counted every folded `chat_id`.

Fix: `is_subagent` ignores self-parent; a group stays visible when it
has any owner spans. Linked subagent activity shows as a violet badge
on the parent row.
...
2. **Create the adapter package** under `cli/internal/coding/hook/<vendor>/`.
   ...
   / `gen_ai.*` schema. Use `setStr(..., scrub)` for any string - never bypass the redaction layer.
...
emit arbitrary `coding_agent.*` spans into the configured collector - including spans claiming to be from a different session, vendor,
