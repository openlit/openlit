...
OpenLIT ships first-class observability for AI coding agents. The `openlit` CLI ingests events from each vendor's hook system, normalizes them onto OpenTelemetry's `gen_ai.*` and OpenLIT's `coding_agent.*` semantic conventions, and ships them to your OpenLIT OTLP endpoint. The [Agents page](/latest/openlit/observability/agents/overview)'s Coding Agents tab detects them automatically - no controller, no SDK, no code change in your repos.
...
<CardGroup cols={2}>
  ...
  <Card title="Scanner connectors" href="/latest/openlit/connectors/scanner" icon='magnifying-glass'>
    Scan the GitHub repo from `vcs.repository.url.full` with Trustabl and review findings on the session
  </Card>
</CardGroup>
...
- The OTLP endpoint of your OpenLIT instance (defaults to `http://localhost:4318` for local installs).
