...
<Steps>
  ...
  <Step title="Wire each vendor">
    ...
    **Optional: Claude Code's native OpenTelemetry exporter.** If you'd rather not install the plugin, or you want a second signal source for cross-checking, you can point Claude Code's built-in OTel exporter at your OpenLIT collector instead. Both paths can run side by side - OpenLIT dedupes per `session.id` (Claude Code's authoritative cost/tokens from the native path win, while the hook path provides the repository/working-folder context that Claude Code itself can't see).
    ...
    # Or install and launch OpenCode in one step
    openlit coding launch opencode
    ```

    Plugin path: `$XDG_CONFIG_HOME/opencode/plugins/openlit.ts` when `XDG_CONFIG_HOME` is set, otherwise `<user-home>/.config/opencode/plugins/openlit.ts`. OpenCode [automatically loads global JavaScript and TypeScript plugins](https://opencode.ai/docs/plugins/#from-local-files) from this directory at startup, so restart OpenCode after installation.

    The plugin projects session lifecycle, completed-assistant metadata, and tool execution into `openlit coding hook --vendor=opencode`. OpenLIT emits canonical LLM-turn and tool-call spans with `coding_agent.client=opencode`. Provider-executed tools, which bypass OpenCode's explicit tool hooks, are recovered from terminal tool-part metadata and deduplicated by call ID.

    OpenCode does not expose a terminal `SessionEnd` hook: `session.idle` is a turn boundary and can also follow errors or cancellation. OpenLIT therefore does not fabricate a completed session root. In `metadata_only` and `full`, it records lifecycle events as they arrive. In `minimal`, it emits non-terminal `coding_agent.session.snapshot.start` / `coding_agent.session.snapshot` spans; token and cost values are per-snapshot deltas, while duration and tool count are cumulative. None of these snapshots carries `coding_agent.session.outcome`.

    OpenCode message text, reasoning text, system messages, and structured parts are never forwarded. Explicit tool arguments and results are included only when `OPENLIT_CODING_CONTENT_CAPTURE=full`; provider-executed fallback events contain terminal metadata only.

    Re-running the install is idempotent and replaces only `openlit.ts`; other files in your global OpenCode plugin directory are preserved.
...
| `$XDG_CONFIG_HOME/opencode/plugins/openlit.ts` when set; otherwise `<user-home>/.config/opencode/plugins/openlit.ts` | ✓ (when `<v>` is `opencode` or `all`) | - |
