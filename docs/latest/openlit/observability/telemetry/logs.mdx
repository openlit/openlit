---
title: "Logs"
description: "A searchable, filterable explorer for OTel log records ingested through OpenLIT's OTLP receiver"
---
...
OpenLIT ingests OTel logs the same way it ingests traces and metrics: through the first-party OTLP receiver (gRPC `:4317`, HTTP `:4318`), which writes them into the `otel_logs` table in ClickHouse. Point your application's log exporter at the same OTLP endpoint you use for traces and metrics to have logs show up here automatically - no separate pipeline is required.
