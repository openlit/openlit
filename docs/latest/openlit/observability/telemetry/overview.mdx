...
The **Telemetry** page is the single home for every OpenTelemetry signal OpenLIT ingests from your AI applications. Navigate to it at `/telemetry`. It has four tabs, all backed by the same ClickHouse tables (`otel_traces`, `otel_metrics_*`, `otel_logs`) populated by OpenLIT's first-party OTLP receiver:
...
<CardGroup cols={2}>
  ...
  <Card title="OTLP receiver" href="/latest/openlit/otlp-receiver" icon='satellite-dish'>
    Send traces, metrics, and logs to OpenLIT over OTLP HTTP and gRPC — no bundled Collector
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="Does OpenLIT support log monitoring, not just traces?">
    Yes. Logs have their own dedicated explorer under Telemetry, ingested the same way as traces and metrics through OpenLIT's OTLP receiver.
  </Accordion>
