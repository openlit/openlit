...
description: 'AI cost tracking: recalculate the cost of LLM traces using current model pricing - manually or on a schedule'
sidebarTitle: 'Cost Recalculation'
---

The Pricing feature is OpenLIT's AI cost tracking engine - it recalculates the `gen_ai.usage.cost` attribute on existing LLM traces using the per-model pricing stored in [Manage Models](/latest/openlit/pricing/manage-models). This is useful when:
...
<AccordionGroup>
  <Accordion title="Why is a trace's cost missing or zero?">
    Usually because the SDK didn't send cost data, or the model wasn't in Manage Models at ingest time. Add the model, then recalculate manually or wait for the next auto-pricing cron run.
  </Accordion>
  <Accordion title="Does auto pricing ever overwrite an existing cost?">
    No. Auto pricing only fills in missing or zero-cost spans - it never overwrites a cost your SDK or an upstream vendor already set, so backfilled and vendor-reported costs never conflict.
  </Accordion>
  <Accordion title="What happens to auto pricing cron jobs after a restart?">
    They're automatically restored from the database on server startup - no manual re-setup is needed after a redeploy or container restart.
  </Accordion>
</AccordionGroup>
