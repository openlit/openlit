---
title: 'Manage Models'
description: 'AI cost tracking foundation: view, edit, and add LLM models with per-model pricing across all providers'
sidebarTitle: 'Manage Models'
---

Manage Models is the AI cost tracking foundation for OpenLIT - it manages LLM provider metadata and per-model pricing, and is used across Chat, Evaluations, OpenGround, and Pricing to look up model details and compute costs.
...
<AccordionGroup>
  <Accordion title="How does OpenLIT calculate AI cost tracking?">
    OpenLIT multiplies input and output token counts from each trace by the per-model pricing stored in Manage Models, giving you cost per request, model, and provider automatically.
  </Accordion>
  <Accordion title="Can I add a custom or self-hosted model?">
    Yes. Add any provider and model with your own pricing, or bulk-import via an SDK-compatible `pricing.json` - this covers self-hosted, fine-tuned, or niche provider models not in the built-in list.
  </Accordion>
  <Accordion title="Does my SDK need to know about pricing changes?">
    No manual sync is required if you use the public pricing URL - the SDK fetches it on startup, so pricing edits in Manage Models propagate automatically on restart or refresh.
  </Accordion>
</AccordionGroup>
