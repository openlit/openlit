...
description: 'Atomic integrations that connect OpenLIT to observability backends, memory stores, and scanners — scoped to projects and environments'
icon: 'plug'
---
...
<Info>
Connectors share one registry model across categories. **Data-source connectors** route telemetry reads; **memory connectors** connect OpenLIT to agent memory stores; **scanner connectors** run Trustabl jobs against GitHub repositories.
</Info>
...
<CardGroup cols={3}>
  ...
  <Card title="Memory connectors" href="/latest/openlit/connectors/memory" icon="brain">
    Claude, Letta, Mem0, and Zep — browse, search, write, and copy agent memories from the Memory page and Otter.
  </Card>
  <Card title="Scanner connectors" href="/latest/openlit/connectors/scanner" icon="magnifying-glass">
    Trustabl — scan GitHub repositories for agent SDK, MCP, and policy findings.
  </Card>
</CardGroup>
...
| Action | Data-source | Memory | Scanner |
| --- | --- | --- | --- |
