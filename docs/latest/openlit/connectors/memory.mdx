...
§ A Letta passage stores only `metadata.tags`. Its create body is `{text, tags}` with no metadata field, so any other key you type into the add dialog's metadata box is not kept server-side. Copies into a Letta connector still keep their port link — OpenLIT stores it on the connector, not in vendor metadata.
