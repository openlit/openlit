...
sidebarTitle: 'Memory'
description: 'Connect OpenLIT to Claude, Letta, Mem0, and Zep memory stores — browse, search, write, copy, and ask Otter about agent memories'
icon: 'brain'
---
...
- **Atomic connectors** — one Mem0 project endpoint, one Zep tenant, one Letta server or Cloud workspace, one Claude memory store configuration per connector instance.
...
- **Port links** — copying a memory to another connector records provenance so you can trace copies back to the source.
...
| **Letta** | `https://api.letta.com` | Browse and search the archival-memory passages a Letta (MemGPT) agent keeps |
...
Self-hosted Letta, Mem0, or Zep deployments work when you point the connector at a compatible API URL and supply the same authentication style as the hosted service.
...
| **Letta** | Yes | Yes | No‡ | Yes§ | No‡ | Yes | No | Agent |
...
‡ No Letta route reads or edits a single passage, and the list cursors are strict — `before`/`after` select the passages around an id, never the id itself — so a passage cannot be addressed on its own. Rather than guess it out of a bounded page and report "not found" for anything older, OpenLIT leaves `get` and `update` unadvertised; the detail sheet shows the record from the list. Edit a passage by deleting it and adding the new text.

§ A Letta passage stores only `metadata.tags`. Its create body is `{text, tags, created_at}` with no metadata field, so any other key you type into the add dialog's metadata box is not kept server-side. Copies into a Letta connector still keep their port link — OpenLIT stores it on the connector, not in vendor metadata.
...
<Steps>
  ...
  <Step title="Pick a vendor">
    Select **Claude**, **Letta**, **Mem0**, or **Zep**.
  </Step>
...
| **Letta** | Bearer (`Authorization: Bearer <apiKey>`) | A Letta Cloud API key, or a self-hosted server's password; an unsecured server needs no key |
...
<CardGroup cols={3}>
  ...
  <Card title="Letta API" href="https://docs.letta.com/api-reference" icon="book">
    Letta agent archival memory and passages.
  </Card>
