...
description: 'Connect OpenLIT to ClickHouse and OpenPlait-backed observability backends — atomic connectors, signal bindings, and portable query adapters'
icon: 'database'
---

**Data-source connectors** attach OpenLIT to observability backends for reading traces, logs, and metrics. Each connector is atomic: one Tempo instance, one Loki instance, one Prometheus, Mimir, or VictoriaMetrics endpoint, one VictoriaLogs instance, one VictoriaTraces instance.
...
These are the data-source connectors available in open-source OpenLIT. OpenPlait packages power the portable query adapters for ClickHouse, Tempo, Loki, Prometheus-compatible metrics, and Jaeger.
...
| **Prometheus** | `@openplait/adapter-prometheus` | metrics | PromQL HTTP API |
| **Grafana Mimir** | `@openplait/adapter-prometheus` | metrics | PromQL HTTP API with Grafana Cloud Basic auth and `X-Scope-OrgID` |
| **VictoriaMetrics** | `@openplait/adapter-prometheus` | metrics | PromQL HTTP API with Victoria `AccountID` / `ProjectID` headers |
| **VictoriaLogs** | `@openplait/adapter-victorialogs` | logs | LogsQL over `/select/logsql/*`; correlate by trace id / service |
| **VictoriaTraces** | `@openplait/adapter-jaeger` | traces | Jaeger Query API at `/select/jaeger`; correlate by trace / span / service |
...
| Mimir | metrics | — | — | Yes | No | — |
| VictoriaMetrics | metrics | — | — | Yes | No | — |
| VictoriaLogs | logs | — | — | Yes | No | trace id, span id, service |
| VictoriaTraces | traces | Yes | Yes | No* | No | trace / span / service |
...
| --- | --- |
| **Grafana Cloud** (Tempo / Loki / Mimir) | Basic: instance ID + access policy token (`traces:read` / `logs:read` / `metrics:read`). Use **query** URLs from the Cloud Portal. |
| **Self-hosted Tempo / Loki / Prometheus / Jaeger** | None, Basic, or Bearer; optional tenant / `X-Scope-OrgID` |
| **VictoriaMetrics / VictoriaLogs / VictoriaTraces** | None, Basic, or Bearer; optional `AccountID` and `ProjectID` headers |
