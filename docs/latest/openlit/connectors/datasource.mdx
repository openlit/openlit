...
**Data-source connectors** attach OpenLIT to observability backends for reading traces, logs, and metrics. Each connector is atomic: one Tempo instance, one Loki instance, one Prometheus, Mimir, or VictoriaMetrics endpoint, one VictoriaLogs instance.
...
| **VictoriaLogs** | OpenLIT LogsQL adapter | logs | LogsQL over `/select/logsql/*`; correlate by trace id / service |
...
| **VictoriaMetrics / VictoriaLogs** | None, Basic, or Bearer; optional `AccountID` and `ProjectID` headers |
