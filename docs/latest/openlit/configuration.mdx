...
`INIT_DB_*` variables **seed** the first Database Config when OpenLIT is first set up. Day-2 host, password, or environment changes belong in **Organisation → Database Config / Connectors**. See also [Connectors](/latest/openlit/organisation/connectors) and [Signal routing](/latest/openlit/organisation/signal-routing).
</Info>
