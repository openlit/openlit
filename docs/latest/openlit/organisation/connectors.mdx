...
The same registry will grow beyond datasources and memory (for example notification and workflow connectors). Each new category still follows: **atomic connector → credentials → actions → optional bindings**. Data-source connectors remain the foundation for telemetry reads.
...
| **Mem0** | API key as `Authorization: Token <key>` |
| **Zep** | API key as `Authorization: Api-Key <key>` |
