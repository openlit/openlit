...
Raw OTLP **write** paths (SDK exporters, optional sidecar collectors) are separate from these **read** connectors. Send OTLP into OpenLIT on `:4317` / `:4318` — see the [OTLP receiver](/latest/openlit/otlp-receiver). To forward from a Collector you operate, use the [sidecar collector example](https://github.com/openlit/openlit/blob/main/assets/otel-collector-config.yaml).
...
<CardGroup cols={2}>
  <Card title="Connectors catalog" href="/latest/openlit/connectors/datasource" icon="plug">
