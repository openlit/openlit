...
Raw OTLP **write** paths (SDK exporters, optional sidecar collectors) are separate from these **read** connectors. To forward OTLP into OpenLIT from your own collector, see the [sidecar collector example](https://github.com/openlit/openlit/blob/main/assets/otel-collector-config.yaml).
