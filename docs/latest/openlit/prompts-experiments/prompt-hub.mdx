---
title: 'Prompt Hub'
sidebarTitle: 'Use Prompt Management'
description: 'Prompt engineering platform for versioning, deploying, and fetching prompts with dynamic variables'
---

Prompt Hub is OpenLIT's prompt engineering platform for managing prompts as versioned artifacts - create, edit, fetch specific versions, and compile prompts with variables at runtime.
      ...
      <Step title="Get prompt using the SDK">
        <Tabs>
            <Tab title="Python">
              ...
              ```
              `url`, `api_key`, `name`, `prompt_id`, `version`, `should_compile`, `variables`, and `meta_properties` are all available parameters - see the [GET Prompt API reference](/latest/openlit/developer-resources/api-reference/endpoint/prompt-hub/get) for the full list.
            ...
            <Tab title="Typescript">
              ...
              ```
              `url`, `apiKey`, `name`, `promptId`, `version`, `shouldCompile`, `variables`, and `metaProperties` are all available parameters - see the [GET Prompt API reference](/latest/openlit/developer-resources/api-reference/endpoint/prompt-hub/get) for the full list.
...
<AccordionGroup>
  <Accordion title="What is prompt engineering with Prompt Hub?">
    Prompt Hub lets you treat prompts as versioned artifacts - edit, version (major/minor/patch), and deploy prompts to any environment without code changes, then fetch them at runtime via SDK or API.
  </Accordion>
  <Accordion title="Can I roll back a prompt version?">
    Yes. Every edit creates a new version, and all versions are retained, so you can fetch or roll back to any prior version instantly.
  </Accordion>
  <Accordion title="How do dynamic variables work in prompts?">
    Use `{{variableName}}` placeholders in a prompt, then pass a `variables` map when fetching it via the SDK or API - OpenLIT compiles the final prompt text at request time.
  </Accordion>
</AccordionGroup>
