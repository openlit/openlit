...
<Steps>
  <Step title="List rules">
    <Frame>
      <img src="/images/rule-engine-list.png" alt="List of rules with name, status, and group operator" />
    </Frame>
...
<AccordionGroup>
  <Accordion title="What is the Rule Engine used for?">
    The Rule Engine matches runtime inputs (like model name or user tier) against conditions you define, then returns linked resources - contexts, prompts, datasets, or evaluation configs - so your application or OpenLIT's evaluators get the right data without hardcoding lookup logic.
  </Accordion>
  <Accordion title="Can I call the Rule Engine from outside OpenLIT?">
    Yes. The evaluate API accepts Bearer token authentication, so any application can evaluate rules and retrieve linked entities without a dashboard session.
  </Accordion>
  <Accordion title="How are multiple conditions combined?">
    Conditions are organized into groups, each with its own AND/OR logic, and groups are combined using a top-level AND/OR operator - so you can express arbitrarily nested matching logic.
  </Accordion>
</AccordionGroup>
