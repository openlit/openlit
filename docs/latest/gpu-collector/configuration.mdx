...
| `OTEL_GPU_EBPF_ENABLED` | `true` on Linux; `false` elsewhere | eBPF CUDA activity tracing + stream-sync occupancy (Linux/NVIDIA only). Soft-fails without caps/CUDA; set `false` to disable |
...
On Linux, eBPF CUDA tracing is **on by default**. It soft-fails (warn + continue) without `CAP_BPF` + `CAP_PERFMON` (or root) and NVIDIA CUDA runtime (`libcudart.so*`). Set `false` to skip the attempt:
