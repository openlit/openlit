...
| `OTEL_RESOURCE_ATTRIBUTES` | `deployment.environment=default` | Resource attributes (`key=val,...`). Prefer setting `host.name`, `k8s.*`, `cloud.provider`, `host.type`, `cloud.region` here — overrides auto-detect |
| `OTEL_METRIC_EXPORT_INTERVAL` | `60000` | Metric polling interval in **milliseconds** |
| `OTEL_GPU_EBPF_ENABLED` | `false` | Enable eBPF CUDA activity tracing + stream-sync occupancy (Linux/NVIDIA only) |
