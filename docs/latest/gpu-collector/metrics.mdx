...
On by default on Linux (`OTEL_GPU_EBPF_ENABLED=true`). Soft-fails without `CAP_BPF` + `CAP_PERFMON` (or root) and an NVIDIA CUDA runtime (`libcudart.so*`). Set `OTEL_GPU_EBPF_ENABLED=false` to disable. Attaches uprobes/uretprobes for launches, alloc/free, memcpy, stream/device sync, and `cudaSetDevice`.
