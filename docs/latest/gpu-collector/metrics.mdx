...
**Prefer** NVML `process.gpu.utilization` for sampled SM%. These estimates are **not** DCGM `hw.gpu.sm.utilization` / `hw.gpu.sm.occupancy`. Use them when correlating with Datadog Agent stream-sync behavior or when NVML process util is unavailable.
