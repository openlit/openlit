...
<CardGroup cols={3}>
  <Card title="GPU Hardware Telemetry" icon="microchip">
    Utilization, memory, temperature, power draw, energy, clock speeds, ECC errors, PCIe errors - for NVIDIA, AMD, and Intel GPUs on Linux and Windows
  </Card>
...
|---|---|---|
| **NVIDIA** | NVML (`libnvidia-ml.so`) | NVML (`nvml.dll`) + PDH process attribution |
| **AMD** | sysfs + hwmon + DRM fdinfo | DXGI + PDH (util/memory/process); thermals/power limited vs Linux |
| **Intel** | sysfs + hwmon + DRM fdinfo | DXGI + PDH (util/memory/process); thermals/power limited vs Linux |
...
GPU Metrics (Linux + Windows)
    +-- Linux: PCI scan → NVML / sysfs / DRM (+ optional eBPF)
