...
<CardGroup cols={3}>
  ...
  <Card title="eBPF CUDA Tracing" icon="code">
    Kernel launch counts, grid/block sizes, memory allocations, and memory copies - via uprobes on libcudart.so (on by default on Linux; soft-fails without caps/CUDA)
  </Card>
</CardGroup>
