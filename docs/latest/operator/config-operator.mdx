...
| Parameter | Description | Default | Example |
|-----------|-------------|---------|---------|
| `observability.enableMetrics` | Enable Prometheus metrics | `true` | `false` |
| `observability.metricsPort` | Metrics server port | `8080` | `9090` |
| `serviceMonitor.enabled` | Create ServiceMonitor | `false` | `true` |
| `serviceMonitor.interval` | Scrape interval | `30s` | `15s` |
| `serviceMonitor.labels` | ServiceMonitor labels | `{}` | `{release: "prometheus"}` |
...
<CardGroup cols={2}>
  <Card title="⚙️ AutoInstrumentation Config" href="/latest/operator/config-autoinstrumentation" icon="settings">
