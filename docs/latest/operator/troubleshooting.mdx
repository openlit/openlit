...
<CardGroup cols={2}>
  <Card title="✅ Healthy Operator" icon="check-circle">
    - Pod status: `Running`
    - Ready: `1/1`
    - Webhook configuration exists
    - Valid TLS certificates
  </Card>
  <Card title="❌ Unhealthy Operator" icon="x-circle">
...
kubectl logs -n openlit deployment/openlit-operator --timestamps=true
```

**Note**: The operator currently uses structured logging for observability. Prometheus metrics endpoint is not yet available.
