...
OpenLIT includes 11 built-in evaluation types (hallucination, bias, toxicity, and more), runs them automatically in production with [Auto Evaluation](/latest/openlit/evaluations/auto-evaluation), and exposes the same evaluators [programmatically via the SDK](/latest/sdk/features/evaluations) for offline testing in CI/CD.
