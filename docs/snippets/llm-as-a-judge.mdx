...
2. Select your **Provider** (OpenAI, Anthropic, Google, TypeSafe, Mistral, etc.) and **Model** to use as a judge
...
<Tip>
Evaluations support chat providers (OpenAI, Anthropic, Google, Mistral, Cohere, Groq, Perplexity, DeepSeek, xAI, Together, Fireworks, and others) plus **TypeSafe Jev** via the native System One API. Jev explanations use the classification bucket plus score; pin `jev-1.13.0` when calibrating thresholds. To trace Jev calls in your application, see [Monitor TypeSafe Jev](/latest/sdk/integrations/typesafe).
</Tip>
