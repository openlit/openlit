...
<CardGroup cols={2}>
  <Card
    ...
    href="/0.1.0/installation#kubernetes"
  >
  ...
  <Card
    title="Docker"
    icon="docker"
    href="/0.1.0/installation#docker"
  >
...
## Docker
Quickly spin up Doku using Docker Compose for a fast and straightforward setup.
...
<Steps>
  ...
  services:
    timescaledb:
      image: timescale/timescaledb-ha:pg14-ts2.14
      container_name: timescaledb
      environment:
        POSTGRES_PASSWORD: ${DOKU_DB_PASSWORD:-DOKU}   
        POSTGRES_USER: ${DOKU_DB_USER:-admin}          
        POSTGRES_DB: ${DOKU_DB_NAME:-postgres}          
      volumes:
        - timescaledb-data:/var/lib/postgresql/data
      ports:
        - "${DOKU_DB_PORT:-5432}:5432" 
      restart: always

    doku-ingester:
      image: ghcr.io/patcher99/doku-ingester:0.1.0
      container_name: doku-ingester
      environment:
        DOKU_DB_HOST: timescaledb   
        DOKU_DB_PORT: ${DOKU_DB_PORT:-5432} 
        DOKU_DB_NAME: ${DOKU_DB_NAME:-postgres}     
        DOKU_DB_USER: ${DOKU_DB_USER:-admin}              
        DOKU_DB_PASSWORD: ${DOKU_DB_PASSWORD:-DOKU}           
        DOKU_DB_SSLMODE: ${DB_DB_SSLMODE:-require}                        
      depends_on:
        - timescaledb
      restart: always
