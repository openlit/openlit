...
<ResponseField name="DOKU_DB_HOST" required>
  Host address for the Clickhouse to be used by Doku
...
<ResponseField name="DOKU_DB_PORT" required>
  Port on which Clickhouse is running
...
<ResponseField name="DOKU_DB_USER" required>
  Username to be used when connecting to Clickhouse
...
<ResponseField name="DOKU_DB_PASSWORD" required>
  Password for the Database user to be used when connecting to Clickhouse
...
<ResponseField name="DOKU_DB_SLLMODE" required>
  SSL Mode to be used when connecting to Clickhouse
