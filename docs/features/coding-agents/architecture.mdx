...
- Adding a new vendor adapter (Claude Code, Codex, Copilot CLI, Windsurf, …).
...
To add another vendor (Copilot CLI / Windsurf / OpenCode / …):
...
6. **Stamp `gen_ai.system`** correctly: `anthropic` for Claude Code,
   `openai` for Codex / Copilot, `google` for Gemini-backed agents.
