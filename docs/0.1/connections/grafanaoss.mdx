...
1.In the settings form, fill in the details to connect to your ClickHouse database:
    - **Server address**: Provide the host address of your ClickHouse Database (e.g., `127.0.0.1`).
    - **Server port**: Provide the port of your ClickHouse Database (e.g., `9000`) and set protocol as `Native`
    - **TLS/SSL Setiing**: Enter the securtity to use for connection. Default used by Doku is `require`.
    - **Username**: The username for the ClickHouse database. Default user used by Doku is `default`.
